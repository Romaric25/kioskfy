...
import { useOrganizationCustomers } from "@/hooks/use-organization-customers.hook";
...
export const OverviewCards = ({ organizationId }: OverviewCardsProps) => {
  ...
  const { data: customersData, isLoading: isLoadingCustomers } = useOrganizationCustomers(organizationId);
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Clients ayant acheté vos journaux</p>
        </CardContent>
      </Card>
    </div>
  );
